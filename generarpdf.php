...
$variableid_materia = $_GET ['id_materia'];
...
//
$queryobteneridgrupo = " SELECT grupo.id_grupo FROM grupo JOIN usuarios WHERE usuarios.usuario LIKE '$usuario'";
$resultadoidgrupo = $conn->query($queryobteneridgrupo);
$row_idgrupo = $resultadoidgrupo->fetch_assoc();
$id_grupo = $row_idgrupo['id_grupo'];

//
$queryobteneridusu= " SELECT usuarios.id_usuario FROM usuarios  WHERE usuarios.usuario LIKE '$usuario'";
$resultadoidusu = $conn->query($queryobteneridusu);
$row_idusu = $resultadoidusu->fetch_assoc();
$id_usuario = $row_idusu['id_usuario'];
...
WHERE calificaciones.id_materia = '$variableid_materia' and alumnos.id_grupo = '$id_grupo' ";
$result = $conn->query($query);

//consulta para obtener el nombre de la materia 
$query_materia = "SELECT materia FROM materias WHERE materias.id_materia = '$variableid_materia'";
...
//variable de id usuarios


 
//generar consulta para obtener grupo
$query_obtenergrupo = "SELECT grupo.grupo_name FROM grupo WHERE grupo.id_usuario = '$id_usuario'";
$result_obtenergrupo = $conn->query($query_obtenergrupo);
$row_grupo = $result_obtenergrupo->fetch_assoc();
$grupo = $row_grupo['grupo_name'];

//generar consulta para obtener nombre de maestro
$queryobtenernombre = "SELECT usuarios.usuario FROM usuarios WHERE usuarios.id_usuario = '$id_usuario'";
$resultnombre = $conn->query($queryobtenernombre);
$row_obtenernombre = $resultnombre->fetch_assoc();
$nombreMaestro = $row_obtenernombre["usuario"];
//generar consulta para obtener turno del maestro 
$queryobtenerturno = "SELECT usuarios.turno_maestro FROM usuarios WHERE usuarios.id_usuario = '$id_usuario'";
$resultturno = $conn->query($queryobtenerturno);
$row_obtenerturno = $resultturno->fetch_assoc();
$turno = $row_obtenerturno ["turno_maestro"];
...
$pdf->Cell(0, 10, "Maestro: $usuario", 0, 1, 'L');
